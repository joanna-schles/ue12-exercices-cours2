...
def add_five(tab):
    tab+=5
    return tab  # 👈 Insérez le code ici
...
def sin_values():
    return np.sin(np.arange(0,2*np.pi+0.1,0.1))  # 👈 Insérez le code
...
def swap_first_rows(arr):
    ...
    arr2[0:1]=arr[1:2]
    return arr2
#np.array(arr[1:2],arr[0:1],arr[2:])
